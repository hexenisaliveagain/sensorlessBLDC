...
void setup() {
  ...
  pinMode(SPEED_UP,   INPUT_PULLUP);  // internall pull-up enable
  pinMode(SPEED_DOWN, INPUT_PULLUP);  //internall pull-up enable
}
// Configure analog comparator ISR 
...
void loop() {
  SET_PWM_DUTY(PWM_START_DUTY);    // Motor startup. Setting initial duty cycle with PWM_START_DUTY
  i = 5000;
  // Motor start
  while(i > 100) {
    delayMicroseconds(i);
  ...
  ACSR |= 0x08;                    // analog comparator interrupt enable
  while(1) {
